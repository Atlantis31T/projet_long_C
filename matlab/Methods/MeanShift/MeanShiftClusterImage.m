function [clustCent,data2cluster,cluster2dataCell] = MeanShiftClusterImage(dataPts,bandWidthG,bandWidthI)%,plotFlag);
...
numClust        = 0;
bandSqG          = bandWidthG^2;
bandSqI = bandWidthI^2;
initPtInds      = 1:numPts;
...
stopThresh      = 1e-1*bandWidthI;                       %when mean has converged
...
while numInitPts
    ...
    while 1     %loop untill convergence
        ...
        sqDistToAllGeom = sum((repmat(myMean(1:2,:),1,numPts) - dataPts(1:2,:)).^2)  ;  %dist squared from mean to all points still active
        sqDistToAllIm = sum(repmat(myMean(3:numDim,:),1,numPts) - dataPts(3:numDim,:)).^2;   %dist squared from mean to all points still active
         indMerge=zeros(numPts,2);
        inIndsG      = find(sqDistToAllGeom < bandSqG);               %points within bandWidth Geometrie
        inIndsI      = find(sqDistToAllIm < bandSqI);  %points within bandWidth Image
        size(inIndsG)
        size(inIndsI)
        indMerge(inIndsG,1)=1;
        indMerge(inIndsI,2)=1;
        inInds=find(sum(indMerge,2)==2);
        size(inInds)
        max(inInds)
...
%         end
        ...
        if norm(myMean-myOldMean) < stopThresh
            ...
            for cN = 1:numClust
                ...
                if distToOther < bandWidthG/2                    %if its within bandwidth/2 merge new and old
                    mergeWith = cN;
                    break;
                end
            end
